...
with st.sidebar:
    ...
    st.divider()
    st.info("👨‍💻 **Author:** Bhargav Kumar Nath\n\n🏗 🛠 **Stack:** H2O, PyTorch, Streamlit, FAISS")
...
def generate_imbalanced_data():
    X, y = make_classification(
        ...
        n_clusters_per_class=1, weights=[0.92, 0.08], flip_y=0, random_state=42
    )
...
if section == "1. Executive Summary":
    st.markdown('<div class="main-header">Learning from Synthetic Data: A Deep Dive</div>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        st.metric(label="Model-Driven AUPRC", value="0.103", delta="0.005", delta_color="normal")
    ...
    st.markdown("The architecture is divided into 3 logical phases: **Ingestion**, **Experimentation (The Core)**, and **Evaluation**.")
    ...
    st.markdown('<div class="main-header">Phase 1: The Imbalance Problem</div>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        * **Imbalance Ratio:** 92% Majority / 8% Minority
        ...
        df_chart = pd.DataFrame({'Class': ['Majority (0)', 'Minority (1)'], 'Count': [63990, 5564]})
        fig = px.pie(df_chart, values='Count', names='Class', title='Target Distribution (Original)', color_discrete_sequence=['#94A3B8', '#EF4444'])
    ...
    with col2:
        ...
        * **Diagnosis:** The model learns a trivial function: *"Predict Majority Always"*.
        ...
        st.markdown('<div class="insight-box">💡 <b>Engineering Insight:</b><br>High accuracy in imbalanced datasets is a vanity metric. We must optimize for <b>AUPRC (Area Under Precision-Recall Curve)</b> to capture the trade-off between detecting the minority class and avoiding false alarms.</div>', unsafe_allow_html=True)

    st.markdown("### 2D Projection of Raw Data")
    df_vis, y_vis = generate_imbalanced_data()
    df_vis['Target'] = y_vis.astype(str)
    fig_scatter = px.scatter(df_vis, x='Feature 1', y='Feature 2', color='Target', color_discrete_map={'0': '#94A3B8', '1': '#EF4444'}, opacity=0.6)
    fig_scatter.update_layout(title="t-SNE Projection of Raw Data (Simulated)")
    st.plotly_chart(fig_scatter, width='stretch')

elif section == "3. The Innovation (Model-Driven)":
    st.markdown('<div class="main-header">Phase 3: The Model-Driven Architecture</div>', unsafe_allow_html=True)
    ...
    with tabs[0]:
        ...
        with col1:
            ...
            st.info("We train a 'Leader' model on the original data. This model approximates the complex, non-linear decision boundary $P(y|x)$.")
            
            st.markdown("#### 2. Vectorized Interpolation")
            st.info("We generate millions of raw candidate points by interpolating between random minority samples. This is lightweight and vectorized.")
        ...
        with col2:
            ...
            st.success("We only retain samples that mathematically conform to the learned physics of the minority class.")
...
while len(synthetic_samples) < TARGET_N:
    ...
    with tabs[2]:
        ...
        with col1:
            ...
            st.markdown("Fills the empty space blindly. Often bridges distinct clusters, creating **No-Man's-Land** noise.")
            # Mock visual
            fig = px.scatter(x=[1, 5], y=[1, 5], title="SMOTE: Blind Interpolation")
            fig.add_shape(type="line", x0=1, y0=1, x1=5, y1=5, line=dict(color="orange", width=2, dash="dash"))
            st.plotly_chart(fig, width='stretch')
            
        with col2:
            st.markdown("#### Model-Driven (Probabilistic)")
            st.markdown("Only fills space where the Oracle says *'Yes, this looks like a valid minority sample'*.")
            # Mock visual
            fig2 = px.scatter(x=[1, 5], y=[1, 5], title="Model-Driven: Curated Generation")
            fig2.add_shape(type="circle", x0=2, y0=2, x1=4, y1=4, line_color="green")
            st.plotly_chart(fig2, width='stretch')

elif section == "4. Latent Space Analysis":
    st.markdown('<div class="main-header">Phase 5: Data Quality QA</div>', unsafe_allow_html=True)
...
class Autoencoder(nn.Module):
    ...
    with col2:
        ...
        X_real, _ = make_blobs(n_samples=200, centers=[[-2, -2], [2, 2]], cluster_std=0.5)
        X_smote, _ = make_blobs(n_samples=100, centers=[[0, 0]], cluster_std=1.5) # Noise in middle
        X_md, _ = make_blobs(n_samples=200, centers=[[-2, -2], [2, 2]], cluster_std=0.7) # Denser clusters
        
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=X_real[:,0], y=X_real[:,1], mode='markers', name='Real Minority', marker=dict(color='blue', opacity=0.5)))
        fig.add_trace(go.Scatter(x=X_smote[:,0], y=X_smote[:,1], mode='markers', name='SMOTE (Noise)', marker=dict(color='orange', symbol='x')))
        fig.add_trace(go.Scatter(x=X_md[:,0], y=X_md[:,1], mode='markers', name='Model-Driven (Quality)', marker=dict(color='green', size=3)))
        
        fig.update_layout(title="Latent Space Projection (Representative Reconstruction)", template="plotly_white")
        st.plotly_chart(fig, width='stretch')
        
        st.markdown('<div class="insight-box"><b>Visual Proof:</b> SMOTE generated samples in the "No-Man\'s-Land" between clusters (Orange). The Model-Driven approach (Green) respected the manifolds of the real data.</div>', unsafe_allow_html=True)

elif section == "5. Robustness & Generalization":
    st.markdown('<div class="main-header">Phase 7: Stress Testing</div>', unsafe_allow_html=True)
    
    col1, col2 = st.columns(2)
    
    with col1:
        st.markdown("### 1. Robustness to Noise")
        st.markdown("We injected random Gaussian noise into the test features to see how brittle the models were.")
        ...
        fig = px.bar(robustness_data, x='Model', y='F1 Drop (%)', color='Model', title='Performance Drop on Noisy Data')
        st.plotly_chart(fig, width='stretch')
        
        st.info("**Interpretation:** Counter-intuitively, the Model-Driven model dropped the *most*. This indicates it learned a **sophisticated, nuanced decision boundary** sensitive to features, whereas the Baseline was \"numb\" (predicting majority regardless of noise).")

    with col2:
        st.markdown("### 2. Generalization ('New World')")
        st.markdown("We tested the models on a completely unseen holdout dataset with slightly shifted distributions.")
        ...
        fig2 = px.line(gen_data, x='Model', y='AUPRC', markers=True, title='Generalization Performance (AUPRC)')
        fig2.update_layout(yaxis_range=[0.09, 0.11])
        st.plotly_chart(fig2, width='stretch')
        
        st.success("**Winner:** The Model-Driven approach generalized best to the New World data, proving that high-quality synthetic data teaches the model the *underlying physics* of the problem, not just memorization.")

elif section == "6. Engineering: Scalability":
    st.markdown('<div class="main-header">Phase 10: Production Readiness</div>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        fig = px.line(df_scale, x='Samples', y='Time (s)', color='Method', title='Runtime Scalability Analysis')
        st.plotly_chart(fig, width='stretch')
        
    st.markdown('<div class="insight-box"><b>Scalability Verdict:</b> The Model-Driven architecture is <b>"Embarrassingly Parallel"</b>. Unlike SMOTE which requires calculating global distances, our filter can run on distributed chunks (e.g., Apache Spark), making it suitable for billion-row datasets.</div>', unsafe_allow_html=True)

elif section == "7. Interactive Playground":
    st.markdown('<div class="main-header">Interactive Oracle Filter Demo</div>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        if st.button("Run Generation Pipeline"):
            ...
            distance_from_center = np.sqrt((candidates[:,0]-0.5)**2 + (candidates[:,1]-0.5)**2)
    ...
    with col2:
        if 'data' in st.session_state:
            fig = px.scatter(
                st.session_state['data'], x='x', y='y', color='Status',
                color_discrete_map={'Accepted': '#10B981', 'Rejected': '#EF4444'},
                title=f"Pipeline Visualization (Yield: {st.session_state['yield']:.1f}%)"
            )
            fig.add_shape(type="circle", x0=0.2, y0=0.2, x1=0.8, y1=0.8, line_color="blue", line_dash="dot")
            fig.add_annotation(x=0.5, y=0.9, text="Oracle High Confidence Region", showarrow=False)
            st.plotly_chart(fig, width='stretch')
        else:
...
# Footer
st.markdown("---")
st.markdown("Built with ❤️ using Streamlit. Based on the 'Synthetic Intelligence' research project.")